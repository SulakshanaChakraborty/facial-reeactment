...
def to_motion_delta(pose_batch, seq_len=64):
    shape = pose_batch.get_shape()
    reshaped = tf.reshape(pose_batch, (-1, seq_len, 2, shape[-1]//2))
    diff = reshaped[:, 1:] - reshaped[:, :-1]
    return tf.reshape(diff, (-1, seq_len - 1, shape[-1]))


def keypoints_to_train(poses, arr):
    shape = poses.get_shape()
    reshaped = tf.reshape(poses, (tf.shape(poses)[0], shape[1].value, 2, 68))
